...
const Timeline = {
    ...
    renderTimeline() {
        ...
        container.innerHTML = events.map((event, idx) => {
            ...
            return '<article class="timeline-event" data-event-id="' + event.id + '" data-index="' + idx + '" style="animation-delay: ' + delay + 's;">'
                + '<div class="timeline-dot" style="background: ' + (epoch.color || '#6B1F1F') + ';">' + (epoch.emoji || '\u2022') + '</div>'
                + '<div class="timeline-card" style="border-left-color: ' + (epoch.color || '#6B1F1F') + ';">'
                + '<div class="timeline-card-date" style="color: ' + (epoch.color || '#6B1F1F') + ';">' + event.date + '</div>'
                + '<h3 class="timeline-card-title">' + event.title + '</h3>'
                + '<p class="timeline-card-short">' + (event.short || '') + '</p>'
                + '<div class="timeline-card-more">Mehr erfahren &rarr;</div>'
                + '</div></article>';
        }).join('');
